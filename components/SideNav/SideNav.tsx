function SideNav({ step, title, message, state }: any) {
  return (
    <div className="relative flex items-start mb-5 space-x-4">
      <div>
        <div
          className={
            state == 'active'
              ? 'absolute inline-flex w-12 h-12 rounded-full opacity-30 animate-ping bg-primary-lightBlue lg:w-9 lg:h-9'
              : ''
          }
        ></div>
        <div
      ...
      <div className="">
        {title && <h3 className="text-xs text-neutral-lightGray UbuntuMedium">{title}</h3>}
        {message && <h4 className="text-sm text-white UbuntuMedium">{message}</h4>}
      </div>
    </div>
  );
}
